...
class JobQueue {
  constructor() {
    ...
    // Loop through the videos and find all the processing true item, and add to the queue(euqueue)
    DB.update();
    DB.videos.forEach((video) => {
      console.log(Object.keys(video.resize)).forEach((key) => {
  ...
  async execute(job) {
    if (job.type === "resize") {
      ...
      try {
        ...
        video.resize[`${width}x${height}`].processing = false;
        DB.save();
        console.log(
